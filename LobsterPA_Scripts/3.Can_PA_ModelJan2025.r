...
saveRDS(list(data=survey,grid=bspde,model=fitpa),file='sdmTMBBerriedpabyQFinal_Can_Jan2025.rds')

x=readRDS(file='sdmTMBBerriedpabyQFinal_Can_Jan2025.rds')

fitpa = x$model
bspde = x$grid
survey= x$data
x = readRDS('CanClimatologiesandDiffsbyQ.rds')
x = x[[2]]
...
ggplot(subset(gC))+#,Q==3 &YEAR %in% 2016)) +
...
saveRDS(gsf,file='Can_CurrentClimatologyBerriedBinomialOutput_Jan2025.rds')

##################################################################
###projections

 glo = readRDS('CanClimatologiesandDiffsbyQ.rds')
 glo = glo[[2]]
...
glo = subset(glo,!is.na(LFA))
 ...
 saveRDS(glo,'CanClimatologiesandDiffsbyQ_withinLFA.rds')
...
gCan30 = aggregate(pred~X1000+Y1000+Q+BT,data=g,FUN=mean)

gCan30 = st_as_sf(gCan30,coords = c("X1000","Y1000"),crs=32620,remove=F)


ggplot(subset(gCan30)) +
  geom_sf(aes(fill=pred,color=pred)) + 
  scale_fill_viridis_c() +
  scale_color_viridis_c() +
  geom_sf(data=rL,size=1,colour='black',fill=NA)+
 ...
 saveRDS(gCan30,file='Can_30sClimatologyBerriedBinomialOutput_Jan2025.rds')
...
gCan50 = aggregate(pred~X1000+Y1000+Q+BT,data=g,FUN=mean)

gCan50 = st_as_sf(gCan50,coords = c("X1000","Y1000"),crs=32620,remove=F)


ggplot(subset(gCan50)) +
  geom_sf(aes(fill=pred,color=pred)) + 
  scale_fill_viridis_c() +
  scale_color_viridis_c() +
  geom_sf(data=rL,size=1,colour='black',fill=NA)+
 ...
 saveRDS(gCan50,file='Can_50sClimatologyBerriedBinomialOutput_Jan2025.rds')
...
gCan90 = aggregate(pred~X1000+Y1000+Q+BT,data=g,FUN=mean)

gCan90 = st_as_sf(gCan90,coords = c("X1000","Y1000"),crs=32620,remove=F)


ggplot(subset(gCan90)) +
  geom_sf(aes(fill=pred,color=pred)) + 
  scale_fill_viridis_c() +
  scale_color_viridis_c() +
  geom_sf(data=rL,size=1,colour='black',fill=NA)+
 ...
 saveRDS(gCan90,file='Can_90sClimatologyBerriedBinomialOutput_Jan2025.rds')

###merge
 gCan90 = readRDS(file='Can_90sClimatologyBerriedBinomialOutput_Jan2025.rds')
 gCan50 = readRDS(file='Can_50sClimatologyBerriedBinomialOutput_Jan2025.rds')
 gCan30 = readRDS(file='Can_30sClimatologyBerriedBinomialOutput_Jan2025.rds')
 gCan   = readRDS(file='Can_CurrentClimatologyBerriedBinomialOutput_Jan2025.rds')
 
 gCan = subset(gCan,select=c(pred,LFA))
 gCan30 = subset(gCan30,select=c(pred))
 gCan50 = subset(gCan50,select=c(pred))
 gCan90 = subset(gCan90,select=c(pred))
 
gCan = bio.utilities::rename.df(gCan,'pred','current')
 gCan30 = bio.utilities::rename.df(gCan30,'pred','thirpre')
 gCan50 = bio.utilities::rename.df(gCan50,'pred','fiftpre')
 gCan90 = bio.utilities::rename.df(gCan90,'pred','ninpre')

g1 = st_join(gCan,gCan30,join=st_intersects)
g1 = st_join(g1,gCan50,join=st_intersects)
g1 = st_join(g1,gCan90,join=st_intersects)
...
#base surface
gs =readRDS(file='Can_CurrentClimatologyBerriedBinomialOutput_Jan2025.rds')
glo = readRDS('CanClimatologiesandDiffsbyQ_withinLFA.rds')
...
saveRDS(hp,'CanSpatialPredictionsBerried_Jan2025.rds')