...
saveRDS(list(data=survey,grid=bspde,model=fitpa),file='sdmTMBBerriedpabyQFinal_MPI_Jan2025.rds')

x=readRDS(file='sdmTMBBerriedpabyQFinal_MPI_Jan2025.rds')

fitpa = x$model
bspde = x$grid
survey= x$data
x = readRDS('MPIClimatologiesDiffsbyQ.rds')
x = x[[1]]
...
saveRDS(gsf,file='MPI_CurrentClimatologyBerriedBinomialOutput_Jan2025.rds')

##################################################################
###projections

 glo = readRDS('MPIClimatologiesDiffsbyQ.rds')
 glo = glo[[1]]
...
glo = subset(glo,!is.na(LFA))
 ...
 saveRDS(glo,'MPIClimatologiesandDiffsbyQ_withinLFA.rds')
...
ggplot(subset(gMPI30)) +
 ...
 saveRDS(gMPI30,file='MPI_30sClimatologyBerriedBinomialOutput_Jan2025.rds')
...
gMPI50 = aggregate(pred~X1000+Y1000+Q+BT,data=g,FUN=mean)

gMPI50 = st_as_sf(gMPI50,coords = c("X1000","Y1000"),crs=32620,remove=F)


ggplot(subset(gMPI50)) +
  geom_sf(aes(fill=pred,color=pred)) + 
  scale_fill_viridis_c() +
  scale_color_viridis_c() +
  geom_sf(data=rL,size=1,colour='black',fill=NA)+
 ...
 saveRDS(gMPI50,file='MPI_50sClimatologyBerriedBinomialOutput_Jan2025.rds')
...
gMPI90 = aggregate(pred~X1000+Y1000+Q+BT,data=g,FUN=mean)

gMPI90 = st_as_sf(gMPI90,coords = c("X1000","Y1000"),crs=32620,remove=F)


ggplot(subset(gMPI90)) +
  geom_sf(aes(fill=pred,color=pred)) + 
  scale_fill_viridis_c() +
  scale_color_viridis_c() +
  geom_sf(data=rL,size=1,colour='black',fill=NA)+
 ...
 saveRDS(gMPI90,file='MPI_90sClimatologyBerriedBinomialOutput_Jan2025.rds')

###merge
 gMPI90 = readRDS(file='MPI_90sClimatologyBerriedBinomialOutput_Jan2025.rds')
 gMPI50 = readRDS(file='MPI_50sClimatologyBerriedBinomialOutput_Jan2025.rds')
 gMPI30 = readRDS(file='MPI_30sClimatologyBerriedBinomialOutput_Jan2025.rds')
 gMPI   = readRDS(file='MPI_CurrentClimatologyBerriedBinomialOutput_Jan2025.rds')
 
 gMPI = subset(gMPI,select=c(pred,LFA))
 gMPI30 = subset(gMPI30,select=c(pred))
 gMPI50 = subset(gMPI50,select=c(pred))
 gMPI90 = subset(gMPI90,select=c(pred))
 
gMPI = bio.utilities::rename.df(gMPI,'pred','current')
 gMPI30 = bio.utilities::rename.df(gMPI30,'pred','thirpre')
 gMPI50 = bio.utilities::rename.df(gMPI50,'pred','fiftpre')
 gMPI90 = bio.utilities::rename.df(gMPI90,'pred','ninpre')

g1 = st_join(gMPI,gMPI30,join=st_intersects)
g1 = st_join(g1,gMPI50,join=st_intersects)
g1 = st_join(g1,gMPI90,join=st_intersects)
...
#base surface
gs =readRDS(file='MPI_CurrentClimatologyBerriedBinomialOutput_Jan2025.rds')
glo = readRDS('MPIClimatologiesandDiffsbyQ_withinLFA.rds')
...
saveRDS(hp,'MPISpatialPredictionsBerried_Jan2025.rds')